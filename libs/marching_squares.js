 ...
 * returns an Array of x and y positions defining the perimeter of a blob of
...
(function(window) {
  ...
  MarchingSquaresOpt.getBlobOutlinePoints = function(
    ...
    if (source_array instanceof HTMLCanvasElement) {
      ...
      const data4 = source_array.getContext('2d')
    ...
    while (startingPoint !== null) {
      ...
      // clear the labeled object in the array
  ...
  MarchingSquaresOpt.getFirstNonTransparentPixelTopDown = function(
      source_array, width, height) {
    let idx;
  ...
  MarchingSquaresOpt.walkPerimeter = function(
      source_array, width, height, start_w, start_h) {

    width = width | 0;
    height = height | 0;

    // Set up our return list
    const point_list = [], up = 1 | 0, left = 2 | 0, down = 3 | 0,
          right = 4 | 0, step_func = MarchingSquaresOpt.step;

    let idx = 0 | 0,  // Note: initialize it with an integer, so the JS
                      // interpreter optimizes for this type.
  ...
  MarchingSquaresOpt.step = function(idx, source_array, width) {
    ...
    // Sakri.imageData =
    // Sakri.MarchingSquaresOpt.sourceContext.getImageData(x-1, y-1, 2, 2).data;

    const up_left = 0 < source_array[idx + 1],
          up_right = 0 < source_array[idx + 2],
          down_left = 0 < source_array[idx + width + 1],
          down_right = 0 < source_array[idx + width + 2], none = 0 | 0,
